 ...
 */
#define LOG(format, ...) project_log(stdout, __FILE_NAME__, __LINE__, format, ##__VA_ARGS__)

/**
 * @brief Panics the message in \p format to stderr and then exits with a non-zero status code
 */
#define LOG_AND_EXIT(format, ...)                                        \
	project_log(stderr, __FILE_NAME__, __LINE__, format, ##__VA_ARGS__); \
	exit(EXIT_FAILURE)
 ...
 */
void project_log(FILE *stream, const char *filename, uint32_t line_no, const char *format, ...);
